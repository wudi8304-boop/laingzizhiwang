...
CREATE TABLE IF NOT EXISTS programs (
 ...
 upload_key TEXT NOT NULL DEFAULT '', upload_key_name TEXT NOT NULL DEFAULT '',
 ...
 completed_at TEXT NOT NULL DEFAULT '', accepted_at TEXT NOT NULL DEFAULT '', settled_at TEXT NOT NULL DEFAULT '',
...
def _looks_like_upload_key(secret):
    text = str(secret or "")
    if len(text) <= 32:
        return False
    return ("BEGIN" in text or "PRIVATE" in text or "\n" in text or "\r" in text)
...
class Database:
    ...
    def initialize(self):
        with self.connect() as conn:
            ...
            for name in (
                "avatar_url", "description", "category", "completed_at", "accepted_at", "settled_at",
            ...
            for row in conn.execute("SELECT id, secret, upload_key FROM programs"):
                secret = row["secret"] or ""
                if row["upload_key"] or not _looks_like_upload_key(secret):
                    continue
                encoded = base64.b64encode(secret.encode("utf-8")).decode("ascii")
                conn.execute(
                    "UPDATE programs SET upload_key=?, upload_key_name=?, secret='' WHERE id=?",
                    (encoded, "upload.key", row["id"]),
                )
            conn.execute(
